...
def RR(M, N, U, V, eta_R1, RefSpeed, sigma):
    ...
    while (len(unKnownQ) != 0 and len(KnownQ) <= sigma):
        unKnownQ = sorted(set(N.index) - set(prior_queries))  # Exclude recovered prior queries.
        tempResult = []
        for query in unKnownQ:
            ...
            try:
                s = -np.log(np.linalg.norm(np.array(Hq.loc[query]) - np.array(Hw), axis=1))
            except ValueError:
                s = 999
...
def scorePlus(M, N, known, RefSpeed):
    ...
    while (len(unKnownQ) != 0):
        unKnownQ = sorted(set(N.index) - set(prior_queries))
        tempResult = []
        for query in unKnownQ:
            # Calculate the Euclidean distance between the query row vector
            ...
            # and take the negative logarithm.
            try:
                s = -np.log(np.linalg.norm(np.array(Nk.loc[query]) - np.array(Mk), axis=1) + 1e-20)
            except ValueError:
                s = 999
