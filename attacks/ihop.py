...
def ihop_attack(ndocs, M, N, wordSet, query, pct_free, n_iters):
    nrep = len(wordSet)
    ntok = len(query)
...
def run_ihop(ndocs, M, N, wordSet, query, pct_free, n_iters):
    tokenToWord = ihop_attack(ndocs, M, N, wordSet, query, pct_free, n_iters)
    result = []
