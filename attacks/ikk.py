# -*- coding: utf-8 -*-
"""
Created on Wed Jul 19 09:07:26 2023
...
def ikk_attack(M, N, wordSet, query):
    naive_flag = False
    unique_flag = True
    cooling_rate = 0.9999
    ...
    if unique_flag:
        keyword_list = list(known_keywords) + list(
            np.random.choice(remaining_keywords, size=len(remaining_tokens), replace=False))
    else:
        keyword_list = list(known_keywords) + list(
            np.random.choice(range(nkw), size=len(remaining_tokens), replace=True))
    initial_state = [kw_id for _, kw_id in sorted(zip(token_list, keyword_list))]

    if len(remaining_tokens) == 0:
        final_state = initial_state
    elif unique_flag:
        final_state = _run_simmulated_annealing(remaining_tokens, remaining_keywords, initial_state, Vexp, Vobs,
                                                unique_flag, cooling_rate=cooling_rate)
    else:
        final_state = _run_simmulated_annealing(remaining_tokens, range(nkw), initial_state, Vexp, Vobs, unique_flag,
    ...
    keyword_predictions_for_each_query = [keyword_predictions_for_each_token[token] for token in range(len(query))]
    return keyword_predictions_for_each_query


def run_ikk(M, N, wordSet, query):
    tokenToWord = ikk_attack(M, N, wordSet, query)
    result = []
    for i in range(len(query)):
        result.append((query[i], wordSet[tokenToWord[i]]))
    return result
