...
ax2 = ax1.twinx()
mean_times = df.groupby(['m', 'attack'], as_index=False)['time'].mean()
for atk in attacks:
    sub = mean_times[mean_times['attack'] == atk].sort_values('m')
    x_pos = [n_to_x[n] + offset_map[atk] for n in sub['m']]
