# -*- coding: utf-8 -*-
"""
Created on Fri Jul 14 16:57:39 2023
...
for i_p, v_p in enumerate(p):
    for i_m, v_m in enumerate(m):
        if i_m == 3:
            continue
        candidate_y_vals = list(df.loc[(i_p * 4 + i_m) * count: (i_p * 4 + i_m + 1) * count - 1]['recovery'])

        yrate.append(np.mean(df.loc[(i_p * 4 + i_m) * count: (i_p * 4 + i_m + 1) * count - 1]['top5']))
...
ax2 = ax1.twinx()
ax2.set_ylabel('Top 5 accuracy', color='r', fontsize=20)
