...
if __name__ == '__main__':
    ...
    map_ihop = {0.25: 75, 0.5: 143, 0.75: 224, 1: 380}
    map_ihopM = {0.25: 75, 0.5: 143, 0.75: 224, 1: 380}
    map_jigsaw_refspeed = {0.25: 1, 0.5: 1, 0.75: 1, 1: 2}
    ...
    for i_m, v_m in enumerate(m):
        for i_count in range(count):
            ...
            t8 = time.time()
            attack = Attacker(M.to_numpy(), N.to_numpy(), 45, 35, map_jigsaw_refspeed[v_m], 1, 0.9)
            ...
            t6 = time.time()
            result5 = ihop.run_ihop(num2, M.values, N.values, wordSet, query, 0.25, map_ihop[v_m])
            ihoptime = time.time() - t6
            acc = utils.accuracy(result5)
            df.loc[len(df)] = [i_count, v_m, 'ihop', ihoptime, acc / len(query)]

            t7 = time.time()
            result6 = ihopM.run_ihop(num2, M.values, N.values, wordSet, query, 0.25, map_ihopM[v_m], R2[:5])
